...
|RTO and RPO failover examples|RPO and RTO are all about trade offs<table><tr><th>Scenario</th><th>Solution</th></tr><tr><td>Very small data loss (RPO- 1min)</br>Very samll downtime (RTO- 5 min)</td><td>**Hot standby** </br>automatically synchronize data</br>have a standby ready to pick up load</br>use automatic failover from master to standby </br> distrubute data and have multiple masters</td></tr><tr><td>Very small data loss (RPO- 1min)</br>BUT I can tolerate some downtimes (RTO-15 min)</td><td>**Warm standby**</br>automatically synchronize data</br>have a standby with minimum infrastructure</br>Scale it up when a failure happens</td></tr><tr><td>Data is critical (RPO- 1min)</br>BUT I can tolerate some downtimes (RTO-few hours)</td><td>Create regular data snapshots and transaction logs </br>Create databases from snapshots and </br>transaction logs when failure happens</td></tr><tr><td>Data can be lost without a problem (for example:cached data)</td><td>Failover to a completely new server</td></tr></table>|
|Data consistency|The problems of having multiple database instances (standbys and replicas)<li>strong Consistency- Synchronous replication to all replicas - will be slow if you have multiple replicas or standbys.  <li>Eventual consitency-Asynchronous replication-A little lag few seconds-before the change is available in all replicas <ul><li>In the intermediate period, different replicas might return different values<li>Used when scalability is more imporatant than data integrity </br> examples: Social media Posts- facebook/twitter/linkedin posts etc..|
|Catogories of databases|Relational (OLTP &OLAP), Document, Key Value, Graph, In Memory among others|
|choosing a type of database|<li>fixed schema/schemaless? <li>Level of transaction properties? (atomicity & consistency) <li>latency? (seconds,milliseconds or microseconds) <li>expected transactions? (hundreds or thousands or millions/second)<li>how much data? (MBs/GBs/TBs/PBs)  |
|Relational Databases|Strong transactional capabilities. Used for <li>OLTP (Online Transaction Processing) use cases  shopping, banking etc.. <li> OLAP (Online Analytics Processing) use cases data warehouses|
|OLTP|Applications where large number of users make large number of small transactions. </br> Use cases Most traditional applications- ERP, CRM,e-commerce, banking </br> Popular databases: MySQL, Oracle, SQL Server etc..<table><tr><th>Recommended Azure Managed Services</th> </tr> <tr><td>**Azure SQL Database:** Managed Microsoft SQL Server</td> </tr><tr><td>**Azure Database for MYSQL:** Managed MySQL</td> </tr><tr><td>**Azure Database for PostgreSQL:** Managed PostgreSQL</td> </tr></table>|
|RB OLTP Azure SQL Database|Azure SQL Database is a cloud-computing database service (Database as a Service DaaS), that is offered by Microsoft Azure Platform which helps to host and use a relational SQL database in the cloud without requiring any hardware or software installation.<li>It is highly available by default. 99.9% availability.<li>Because this database is managed, it is automatically backed up for us and automatic updates. <li> Flexible and responsive serverless compute <li>Hyperscale (up to 100TB) storage. </br>Typical uses for the SQL database include any company database like HR, maybe inventory or payroll.|
|RB OLTP Azure Database for MySql|Similar to the Azure SQL database.Fully managed, scalable<li>This is also a relational database service with 99.9% availability <li> Choose single zone/zone redudant high availability <li>The difference here it supports community-available editions.<li> automatically backed up and updates </br> Typically used as part of LAMP (Linux, Apache, MySQL, PHP/Perl/Python) stack  |
|RB OLTP Create a MySQL server|</br> <ol> <li>search for Mysql <li> Create - option single/flexible server<li> subscription, resource group, server name, default location, default version, pricing tier - basic, general purpose, moemory optimized <li> admin username, password, review and create<pre>mysql --host=mysql-server-in28minutes.mysql.database.azure.com</br>--user=mysqlserver@mysql-server-in28minutes -p </br>create database todos;</br>use todos;</br>create table user (id integer, username varchar(30) );</br>describe user; </br>insert into user values (1, 'Ranjith'); </br>insert into user values (2, 'Ramesh'); </br>select * from user;</pre>|
|RB OLTP Azure Database for PostgreSQL|And the same holds true for the Azure Database for PostgreSQL.<li>Fully managed, intelligent and scalable<li>It is also a relational database service with 99.9% availability <li> Choose single zone/zone redudant high availability <li>It's available in a community edition <li> automatically backed up and updated <li> Single Server and Hyperscale options. Hyperscale: scale to hundresds of nodes and execute qeries across multiple nodes |
|RB OALP|Applications allowing users to analyse petabytes of data. Data can be consolidated from multiple transactional databases. </br>e.g Reporting applications, Dataware houses, Business intelligence applications, Analytics system|
|RB OALP Recommended Azure managed services|<li>Azure Synapse Analytics (old name Azure SQL data warehouse) ) :Petabyte-scale distributed dataware house <ul><li>Provides a unfified experience for devloping end-to -end analytics solutions- Data integration + </br> Enterprise data warehousing + Big data analytics <li>Enables MPP (massively parallel processing) - data is spread across multiple nodes <li> Run complex queries across petabytes of data|
|RB -<br> OLAP vs OLTP|OLAP and OLTP use similiar data structures </br> BUT very different approach in how data is stored <ul><li>OLTP use row storage - each table row is stored together, efficient for processing small transactions <li> OLAP databases use cluminar storage - each table column is stored together ( high compression-store petabytes of data efficiently </br> Distribute data- one table in multiple cluster nodes, Execute single query across multiple nodes- Complex queries can be executed efficiently|
|NoSQL databases|Flexible schema- - lete the schema evolve with time </br> horizontally scale to petabytes of data with millions of TPS </br> scalability and high performance </br> Azure managed service is Azure cosmosDB|
|CosmosDB|Fully managaed NoSQL database service </br>Global databse: Automatically replicates data across multiple Azure regions <li> Schemaless <li> Single-digit millisecond response times </br> 99.999 -percent availability </br> Automatic scaling (serverless) </br> Supports APIs for MongoDB (document), Cassandra (key/value) and Gremlin (graph)|
|Set up a Cosmosdb instance|Create Azure Cosmos DB account </br> resource group, name, API (core,MongoDB,Cassandra, Azure Table, Gremlin), defaults for the rest of the things. </br> options for global distribution review and create</br> go to resource |
|In memory databases |Retrieving data from memory is much faster than retrieving data from disk, micro seconds latency. </br> Use cases Caching, session management, gaming leader boards, geospatial applications </br> Recommended Azure Managed Service<li> Azure Cache for Redis </br> Use Cases: Caching, session management, gaming leader boards, geospatial applications|
|Databases - Summary|<table><tr><th>Database type </br> Transactional usecases </th> <th>Azure Services</th></tr><tr><td>Relational OLTP databases </br></br>needing predefined schema and very </br> strong transactional capabilities</td><td>Azure SQL Database, Azure Database for MySQL </br> Azure Database for PostgresSQL etc.</td></tr><tr><td>Relational OLAP databases </br> Columnar storage with predefined schema.</br>Datawarehousing & BigData workloads</td><td>Azure Synapse Analytics</td></tr></tr><tr><td>NoSQL Databases </br> Apps needing quickly evolving structure (schema-less)</td><td>CosmosDB </br> supports MongoDb, Cassandra, Gremlin</td></tr><tr><td>In memory databases/caches </br>Applications needing microsecond reponses</td><td>Azure Cache for Redis</td></tr></table>|
|Azure Database scenarios|<li>A start up with quickly evolving schema - CosmosDB <li> Single-digit millisecond reponse times for global application with millions of users - CosmosDB <li>Transactional local database processing thousands of transactions per second - RB- Azure SQL database/Azure MYSQL/Azure PostgresSQL <li> Cache data (from database) for a web application - Azure Cache for Redis <li>Database for analytics processing of petabytes of data - Azure Synapse Analytics|
 </font> 
 ...
 <font size="1">
  ...
  | Key word     | Description |
| ----------- | ----------- |
|Need for a Virtual Network|Corporate network provides a secure internal network protecting your resources,</br> data and communication from external users|
|Azure Virtual Network| your own private network in the cloud </br> Network traffic within a virtual network is isolated from all the other azure virtual networks </> created in a region one VN is assosiated in one region </br> You control all the traffic incoming and outgoing </br> Best Practice to create all your Azure resources (compute, storage, databases etc) within virtual network <li> Secure resources from unauthorized access AND <li> Enable secure communications between your cloud resources|
|Subnets|inside the virtual network you can create multiple subnets </br> **Need for a subnet** <li>Different resources are created on cloud/VN. Each type of resource has its own needs </br> i.e. Load Balancers are accessible from internet (public) </br> Databases or VM instances should NOT be accessible from iternet </br> How do you separate public resources from private resources inside a VNet? <li> Create different subnets for public and private resources <li> Resources in a public subnet CAN be accessed from internet <li> Resources in a private subnet CANNOT be accessed from internet <li>BUT resources in public subnet can talk to resources in private subnet|
|Create Subnet in a VN| Create VNet, subscription, resourcce group, instance details, </br> default subnet is created by default. </br> also can add a subnet and assign the default cdr to this subnet </br> create a VM in the Vnet using the same resource group (then Vnet would be automatically selected, now you can use the subnet)|
|Virtual Networks points| <li> Every VM in a VNet is assigned a private IP address by default. <li> you can choose to have a public ip address as well <li> you can make public ip address either Dynamic or Static <li>private ip addresses are free but public ip addresses cost <li> VMs in the same VNet cancommunicate using private IP addresses - even if they are in differnt subnets|
|Network peering|Network peering is when one internet network connects to another directly, </br> enabling a faster throughput and exchange of information. </br>No additional charges are incurred and no third-party network is required. </br>The typical connection to the internet is called transit. </br> Peered Virtual Networks can be in different regions|
|create a peering relationship|within VNet -> peerings -> add peering|
|Azure DDoS|Large scale attacks to bring your apps down. App goes down or become slow. </br> Hige bill because of unlimited auto scaling </br>By default, the basic skew is included in your Azure subscription. If you find that basic protection is not enough, there is a standard skew that enhances protection for VNet resources.</br> two Azure DDos offerings: <li> DDos Protection **Basic (default):** </br>Protects against common network layer attacks. </br> intelligently identifies and blocks DDos attacks </br>Enabled by default with no extra costs. <li> DDoS Protection **Standard (paid):** </br> Enable for additional cost </br>Mitigares 60 different DDoS attack types </br>Provides attack analytics, metrics, alerting and reporting </br> Get quick support from DDoS Protection Rapid Response (DRR) team </br>Get a Cost gurantee (Recieve service credit if DDoS attack wasn't prevented abd results in scale out </br> Enable it on the Azure virtual network|
|Azure firewall </br> external firewall </br> i.e outside your VN| Managed network security to control traffic in and out AVN </br>This is a stateful firewall. </br>Stateful: Once traffic in is allowed, traffic out is automatically allowed</br> Centralized configuration: with one Azure firewall, you can control traffic to multiple virtual networks across multiple subscription </br>It's used to control access to Azure apps and resources again using allow or deny. </br> The Azure firewall can filter both inbound and outbound traffic.</br> Integrates with Azure Monitor: Provides loggin and analytics </br>There are two types of rules that can be applied in the firewall. <li>Application rules, which use a fully qualified domain name or FQDN (For example, only traffic to abc.com is allowed or traffic for X, Y, Zed is denied.) <li>You can also apply network rules to your firewall. These rules look very similar to what we just saw in the network security group rules. They use a source and target address, protocol, destination port, and destination address.|
|Azure firewall VS web application firewall| WAF restrics traffic into web applications OWASP etc.|
|Create a firewall|your virtual network -> firewall (under setting) ->add a firewall</br> Once a firewall is created, it can be shared with multiple VNs |
|Network Security Groups (NSG) </br> internal firewall|Multiple inbound and outbound security rules: <li> Allow orblock traffic based on source/destination IP address, protocol and port <li>Restrict traffic between resources such as virtual machines and subnets <li>Attached with subnets and network interfaces. </br> Use cases: </br>&nbsp; &nbsp; allow access to webserver only on port 80 and port 443 (HTTP/HTTPS) </br>&nbsp; &nbsp; Restrict access only to web servers. Do NOT allow direct access to database from outside world/other servers </br>&nbsp; &nbsp; Restrict outbound traffic from VMs to download software packages and system updates|
|Configure network security group|create a resource ->create VM (your can see Vn-Vm-nsg is created)-> download the generated key value pair-> click link Vn-Vm-nsg/ or by typing nsg in the search bar-> inbound/outbound rules|
|Security Best Practice </br> secure at all levels| <li> **Perimeter:**  Azure DDos Protection + Azure fire wall <li> **Network:** Restrict internet access (inbound and outbound). i.e restrict communicaton between resources <li> **Compute:** Secure access to VM. i.e.implenet endpoint protection, ensure that OS and software bpatches are applied <li>**Application:** Thisnk of security from day one! i.e. Implement security best practices depending on language and framework, Store secrets in Azure Key vault <li> **Data:** Encrypt data at rest and in transit. <li> **Best Practice:** Implement security at all levels |
|Public vs Private vs Hybrid|**Public cloud** </br> <li> host everything in the cloud. no data center required, no capital expenditure required <li> Hardware resources are owned by Azure Microsoft and shared between tenants </br>. hardware failures and security of the data center are managed bu Azure. </br> **Private Cloud** </br> <li> host everything in your own data center- needs capital expenditure, incur staffing and maintenance expenses for infrastructure <li> delivers high level of security and privacy e.g government </br> **Hybrid cloud** </br><li>Combination of private and public <li> Use public cloud for some workloads and private cloud for others </br> e.g. connecting an on-premise app to Azure Cosmos DB <li> provide you with flexibility: go on-premises or cloud based on specific requirement <li> however adds additional complexity. ie implementing single signon in both sights.| 
|Hybrid cloud in detail| Connecting Azure with on-premises. Options VPN and Azure ExpressRoute |
|VPN |encrypted connection.</br> send encrypted traffic between an Azure virtual network and an on-premises location over the public Internet </br> Needs VPN **device or gateway** on - premises, Need Azure VPN gateway in the azure virtual network|
|Azure ExpressRoute| if private connectivity and higher bandwith required. provides high security </br>
traffic is not encrpted by the connection (can still done by the application) |
|Summary|The key things to remember here are the services that can be used to protect your virtual networks, </br> those being the network security group, the Azure firewall, and DDoS protection. |
 ...
 <font size="1">
  ...
  | Key word     | Description |
| ----------- | ----------- |
 |Organize your Azure resources effectively|Organize your cloud-based resources to secure, manage, and track costs related to your workloads. </br>To organize your resources, define a management group hierarchy, consider and follow a naming convention, </br>and apply resource tagging.|
 |Azure resource Hierarchy | </br>![alt text](Azure_resource_Hierarchy.png)  </br> A **management group** can have a multiple subscriptions </br> Each **subscriptions** has multiple resource groups </br> and each **resource group** has multiple resources|
 |Resources|  VMs, Storage, Databases etc.|
 |Resource groups|Organise resources by grouping them into Resource groups. delete resource group, all the resources will be deleted|
 |Subscriptions|Manage costs for resources provisioned for differnet teams or projects or different business units. ie. it is liked to the cost directly|
 |Management groups|Centralized management for access, policy and compliace across multiple subscriptions</br>important!!! Management groups can have hierarchy. i.e another management groups inside a  management group (no hierarchy in resource groups btw)|
 |IMPORTANT </Br>RESOURCE GROUP </br>facts|<li> A RESOURCE GROUP is a logical container for resources, can have multiple resources<li>One RESOURCE GROUP is attached to one SUBSCRIPTION <li> you can change the subsctiption from one to another if you have multiple <li> A RESOURCE can be associated with one and only RESOURCE GROUP <li> Deleting it delete all resources under it <li>Tags assigned to resource group are NOT automatically applied to resources<li> However, permissions/Roles assigned to user at the resource group level are inherited by all resources in the group <li> Resource groups (like management groups) are free |
 |IMPORTANT </Br>SUBSCRIPTIONS </br>facts|<li> you need a subscription to create resources in Azure<li> subscription liks Azure account to its resources <li>An Azure account can have multiple subscriptions and multiple account administrators. <li> when to creat a new subscription? </br> when you want to manage different access-management policies for differentenviroments ie dev, prod etc </br> &nbsp;&nbsp; -  create different sunscription for diffrent environemnts </br> &nbsp;&nbsp; - Manage distinct Azure subscription for each environmentn </br> When you want to manage costs across different departments of an organization</br> &nbsp;&nbsp; - create different subscriptions for different departments </br> &nbsp;&nbsp; - create separate billing reports and invoices for each subscription (or department) and manage costs </br> when you are exceeding the limits avaialble per subscription (quota) <li> Two subscriptions <ins>cannot be </ins> merged to one <li> However, you can move resources from one to another. <li> ownership of the subscription can be transfered from one owner to another <li> if subscription expires, you <ins>will not be </ins> able to create new resources in the subscription , but you can continue to access </br> the data stored <li> Each subscription is associate with quotas - you can raise a support request to increase some of the quotas <li> You can convert a **Free Trial**  to a **Pay as you go** subscription. <li> Subscription spending limit, prevents spending over your credit limit - free as well as spending limits </br> You can't change spending limit BUT you can remove it! <li> You need an Azure Active Directory account to manage a subscription, not a Microsoft account. </br> An account is created in the Azure Active Directory when you create the subscription.|
 |IMPORTANT </Br>MANAGEMENT GROUP </br>facts|<li>Allows you to manage access, policies, and compliance across multiple subscriptions <li> Group subscriptions into Management groups <li>All subscriptions & resources under a management Group inherit all constraints applied to it  <li> You can create a hierarchy of management groups <li> All subscriptions in a management group should be associated with the same Azure AD tenant|
...
|You have a requirement to keep your files for period of 5 years. These files will be rarely accessed.</br> What storage tier would you recommend?|Archive|
|You have several machines running on-premises with Windows 10. </br>You are planning to map network drive to these machines and you been tasked to choose corresponding Azure Service. </br>Which of these options would you recommend?|Azure File Service|
...
|You are runiing an application in VM instance storing its data on a persistent data storage.</br> You are taking snapshots every 48 hours. If the VM crashes,</br> you can manually bring it back up in 45 minutes from the sanpshot. What is your RTO and RPO?|RTO-45 min </br>RPO-48 hours|
|Azure Cosmos DB is a |Non Relational or NoSQL Database|
|You want to create a managed relational database using Microsoft SQL Server in Azure. </br> Which of these services would you recommend?|Azure SQL Database|
|Which of these is a Global database that automatically replicates data across multiple Azure regions?|Azure Cosmos DB|
|Which of these Azure services is recommended for Datawarehousing & BigData workloads?|Azure Synapse Analytics|
|Which of these services offers an in-memory database?|Azure Cache for Redis|
|how to control traffic if your enterprise has 10 VNs across multiple subscription with 100vms?| One Azure fire wall|
|Which of these help you to create your own isolated private network in Azure?|Virtual|
|You are running a web application on an Azure VM. </br>You want to ensure that the application is accessible from internet by opening up access to ports 80 and 443 on the VM.</br> Which of these would you need to configure?|Network Security Groups|
|TRUE or FALSE: You have created a Virtual Network with two subnets - </br> WebSubnet for deploying your VMs, DbSubnet for deploying your databases.</br> By default, all VMs in WebSubnet can access databases created in DbSubnet.|True|
|TRUE or FALSE: All virtual machines in a Virtual Network can communicate with other machines </br> inside the same Virtual Network irrespective of the subnets they are in.|true|
|Can you establish VNet Peering between virtual networks in different regions?| Yes|
|Which of these are large scale attacks to bring your apps down?|DDos attacks|
|Which of these is a paid Azure service to mitigate 60 different DDoS attack types?|DDos Protection standard|
|Which of these is a Managed network security service to control </br>traffic going in and out of multiple Azure Virtual Networks?|Azure firewall|
|Which of these allow you to allow or block traffic inside a Virtual Network based on  </br> source/destination IP address, protocol and port?|Network Security Groups (NSG)|
|Which of these allow you to restrict outbound traffic from VMs to download software packages and system updates?|NSG|
|We do NOT want to use a private datacenter to host infrastructure, </br>web applications and databases for in28minutes.com. What is the recommended option?|Public Cloud|
|We are using a private datacenter to host infrastructure, web applications and databases for in28minutes.com. </br>We would want to expand further by using infrastructure provided by Azure. Which model are we expanding to?|Hybrid Cloud|
|We are using a private datacenter to host infrastructure and web applications for in28minutes.com. </br>We are hosting our databases in Azure. Which model are we making use of?|Hybrid Cloud|
|In __________ cloud, all the resources/ infrastructure/ data center are owned by third party. </br>This infrastructure is shared with multiple organizations.| Public|
|We are using a private datacenter to host infrastructure, web applications and databases for in28minutes.com. </br>We are moving all our infrastructure to Azure. After migrating to Azure, which of these will NOT be a </br>responsibility of the in28minutes team?|Manging physical server security|
|Which of these is an encrypted connection from on-premises to Azure over internet?|Azure VPN|
|Which of these provides a high-bandwidth private connectivity from on-premises to Azure Virtual Network?|Azure ExpressRoute|
|TRUE or FALSE: There can be NO hierarchy in resource groups BUT management groups can have a hierarchy|True|
|TRUE or FALSE: You can convert a Free Trial subscription to a Pay as you go subscription|True|
|TRUE or FALSE: Resource Group is a logical container for all resources|True|
|Which of these provides centralized management for access, policy, and compliance across multiple subscriptions?|Mangement groups|
|You have created a Resource group in Region - West Europe. Can you provision a resource (say, a VM) in a different region and attach it with the resource group?|yes|
|TRUE or FALSE: One Virtual Machine can be mapped to multiple Resource Groups|False|
|TRUE or FALSE: Deleting a resource group will delete all resources attached with the resource group|True|
|TRUE or FALSE: Azure resources in one Resource Group can access resources in other Resource Groups|True|
|Permissions/Roles assigned to user at the resource group level are applied/inherited to all resources in the group|True|
|I want to manage costs individually for different departments of an organization. </br> Each department needs to have separate account administrators. Which of these options would you recommend?|create a separate </br>subscription for</br> each department|
|TRUE or FALSE: You have created 10 resource groups in a pay-as-you-go subscription. You will be billed for Resource Groups.|false|
|TRUE or FALSE: One resource group can be associated with Multiple Subscriptions|false|
|Can two subscriptions belonging to same organization be merged into one?|No|
|Can an owner of an subscription transfer ownership of the subscription?|yes|
|TRUE or FALSE: Azure Subscriptions can only be created using Microsoft accounts|False|
|TRUE or FALSE: Azure Subscriptions can have more than one account administrators|true|
|TRUE or FALSE: You have created an Azure SQL database instance using Free Tier Subscription. </br>You have now upgraded to pay-as-you-go subscription. </br>To use the database, you need to move the database to the new subscription.|false|
   </font>
